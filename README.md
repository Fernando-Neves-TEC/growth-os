...
- **GAUNTLET V2 (hardening — ver `docs/SPRINTS.md`):**
  ...
  - **SECURITY HARDENING FINAL (resíduos):** timing equalization no login (F-01) · API key timing-safe `timingSafeEqual` (F-07) · email max 254 (F-02) · health público sanitizado sem `killSwitch` (F-09) · 429 genérico + sem `X-Powered-By` (F-11/F-14) · RBAC documentado como admin-único com `CHECK` (F-10) · CORS ACAC documentado (F-03) · topologia single-instance/trust-proxy/multi-instance documentada (F-04/F-05) · `npm audit` 0 CRITICAL runtime, 2 HIGH não alcançáveis (BLOCKED_DECISION).
