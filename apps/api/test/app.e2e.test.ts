...
const ADMIN_PASSWORD = "SenhaTeste123!";
...
describe("Growth OS API (e2e)", () => {
  ...
  beforeAll(async () => {
    ...
    // S2: identidade/sessão em memória + operador de teste + login (sessão humana).
    ...
    const moduleRef = await Test.createTestingModule({ imports: [AppModule] })
      ...
      .useValue(leadStore)
      .overrideProvider(OPERATORS_STORE)
      .useValue(new MemoryOperatorsStore())
      .overrideProvider(SESSION_STORE)
      .useValue(new MemorySessionStore())
      .overrideProvider(SECURITY_AUDIT_STORE)
      .useValue(new MemorySecurityAuditStore())
    ...
    await auth.createOperator(ADMIN_EMAIL, ADMIN_PASSWORD);
    agent = request.agent(app.getHttpServer());
    const login = await agent.post("/auth/login").send({ email: ADMIN_EMAIL, password: ADMIN_PASSWORD }).expect(201);
  ...
  // mutação autenticada por sessão + CSRF
  const post = (path: string, body: unknown) => agent.post(path).set("X-CSRF-Token", csrf).send(body);

  it("GET /campaigns começa vazio (sessão humana)", async () => {
    const res = await agent.get("/campaigns").expect(200);
  ...
  it("GET /leads começa vazio (sem runs de pipeline)", async () => {
    ...
    const runs = await agent.get("/leads/runs").expect(200);
  ...
  it("POST /campaigns cria campanha com workflow válido", async () => {
    const res = await post("/campaigns", { name: "fisio", workflow: validWorkflow }).expect(201);
  ...
  it("POST /campaigns rejeita workflow inválido (fail-closed)", async () => {
    const res = await post("/campaigns", { name: "bad", workflow: { ...validWorkflow, entry: "inexistente" } }).expect(400);
    expect(res.body.message).toContain("workflow inválido");
    expect(res.body.errors.length).toBeGreaterThan(0);
  });

  it("POST /campaigns/:id/plan-day planeja com warm-up", async () => {
    const created = await post("/campaigns", { name: "p", workflow: validWorkflow }).expect(201);
    const leads = Array.from({ length: 20 }, (_, i) => ({ leadId: `l${i}`, channel: "whatsapp", body: `m${i}` }));
    const res = await post(`/campaigns/${created.body.id}/plan-day`, { leads, dayIndex: 0 }).expect(201);
  ...
  it("POST /campaigns/:id/simulate-turn roda a state machine", async () => {
    const created = await post("/campaigns", { name: "s", workflow: validWorkflow }).expect(201);
    const res = await post(`/campaigns/${created.body.id}/simulate-turn`, { event: { type: "start" }, currentNode: null }).expect(201);
  ...
  it("kill-switch pausa e bloqueia planejamento (fail-closed)", async () => {
    const created = await post("/campaigns", { name: "k", workflow: validWorkflow }).expect(201);
    await post("/channels/pause", { reason: "rejeicao_alta" }).expect(201);
    await post(`/campaigns/${created.body.id}/plan-day`, { leads: [{ leadId: "x", channel: "whatsapp", body: "oi" }], dayIndex: 0 }).expect(409);
    await post("/channels/resume", {}).expect(201);
  });

  it("GET /metrics/funnel expõe ARR projetado e parâmetros parametrizados", async () => {
    const res = await agent.get("/metrics/funnel").expect(200);
  ...
  it("POST /events é idempotente (eventId único, efeito único)", async () => {
    const before = (await agent.get("/metrics/funnel").expect(200)).body.counters.sent;
    const body = { eventId: "evt-idem-1", type: "sent" };
    const r1 = await post("/events", body).expect(201);
    expect(r1.body.duplicate).toBe(false);
    const r2 = await post("/events", body).expect(201);
    expect(r2.body.duplicate).toBe(true);
    const m = await agent.get("/metrics/funnel").expect(200);
  ...
  it("POST /events rejeita evento fora de ordem com 422 (invariante do funil)", async () => {
    await post("/events", { eventId: "inv-sent", type: "sent" }).expect(201);
    const res = await post("/events", { eventId: "inv-read", type: "read" }).expect(422);
  ...
  it("POST /events tipo optout adiciona à suppression", async () => {
    await post("/events", { eventId: "evt-opt-1", type: "optout", cnpj: "11222333000181" }).expect(201);
    const res = await agent.get("/suppression").expect(200);
    expect(res.body.some((s: { cnpj: string }) => s.cnpj === "11222333000181")).toBe(true);
  });

  it("POST/GET /suppression registra e lista opt-out", async () => {
    await post("/suppression", { cnpj: "12.345.678/0001-95", reason: "auditoria" }).expect(201);
    const res = await agent.get("/suppression").expect(200);
    expect(res.body.some((s: { cnpj: string }) => s.cnpj === "12345678000195")).toBe(true);
  });

  it("POST /suppression rejeita CNPJ inválido (400)", async () => {
    await post("/suppression", { cnpj: "12.345.678/0001-90" }).expect(400);
    const res = await agent.get("/suppression").expect(200);
  ...
  it("CAMINHO PRINCIPAL: criar → iniciar via API → workflow → suppression → eventos → contadores → métricas (C1)", async () => {
    ...
    await post("/suppression", { cnpj: "00000000000191" }).expect(201);
    const created = await post("/campaigns", { name: "main", workflow: validWorkflow }).expect(201);
    const before = (await agent.get("/metrics/funnel").expect(200)).body.counters;
    const plans = [
    ...
    const start = await post(`/campaigns/${created.body.id}/start`, { plans }).expect(201);
    expect(start.body.status).toBe("active");
    expect(start.body.workflowId).toBe(`fake-${created.body.id}`);
    const after = (await agent.get("/metrics/funnel").expect(200)).body.counters;
    ...
    const detail = await agent.get(`/campaigns/${created.body.id}`).expect(200);
    expect(detail.body.status).toBe("active");
    // dashboard consegue consultar resultado consolidado
    const st = await agent.get("/status").expect(200);
    expect(st.body.leads).toBeGreaterThanOrEqual(0);
  });

  it("POST /campaigns/:id/start em campanha inexistente → 404 (H3)", async () => {
    await post("/campaigns/00000000-0000-4000-8000-000000000000/start", { plans: [{ leadId: "l", channel: "whatsapp", body: "oi" }] }).expect(404);
  });

  it("POST /campaigns/:id/start rejeita plans inválidos (400)", async () => {
    const created = await post("/campaigns", { name: "p2", workflow: validWorkflow }).expect(201);
    await post(`/campaigns/${created.body.id}/start`, { plans: [] }).expect(400);
    await post(`/campaigns/${created.body.id}/start`, { plans: [{ leadId: "l", channel: "sms", body: "oi" }] }).expect(400);
  });

  it("POST /campaigns/:id/start bloqueado pelo kill-switch → 409 (fail-closed)", async () => {
    const created = await post("/campaigns", { name: "p3", workflow: validWorkflow }).expect(201);
    await post("/channels/pause", { reason: "test" }).expect(201);
    await post(`/campaigns/${created.body.id}/start`, { plans: [{ leadId: "l", channel: "whatsapp", body: "oi" }] }).expect(409);
    await post("/channels/resume", {}).expect(201);
  });

  it("H3: GET /campaigns/:id inexistente (UUID válido) → 404", async () => {
    await agent.get("/campaigns/00000000-0000-4000-8000-000000000000").expect(404);
  });

  it("H3: GET /campaigns/:id com UUID inválido → 404 (não 500)", async () => {
    await agent.get("/campaigns/nao-e-um-uuid").expect(404);
  });

  it("H3: plan-day em campanha inexistente → 404", async () => {
    await post("/campaigns/00000000-0000-4000-8000-000000000000/plan-day", { leads: [{ leadId: "l", channel: "whatsapp", body: "oi" }] }).expect(404);
  });

  it("H4: validação estruturada — evento com tipo inválido → 400 com erros", async () => {
    const res = await post("/events", { eventId: "e-bad-1", type: "nao-existe" }).expect(400);
    expect(res.body.errors.length).toBeGreaterThan(0);
  });

  it("H4: validação estruturada — campanha sem name → 400", async () => {
    await post("/campaigns", { workflow: validWorkflow }).expect(400);
  });

  it("H4: validação estruturada — query inválida em /leads → 400", async () => {
    await agent.get("/leads?limit=abc").expect(400);
  });

  it("H6: payload acima do limite de body → 413", async () => {
    const big = "x".repeat(8 * 1024);
    await post("/events", { eventId: "big-1", type: "sent", cnpj: big }).expect(413);
  });

  it("CORS em simulação reflete a origem (dev local)", async () => {
    const res = await agent.get("/status").set("Origin", "http://localhost:5173").expect(200);
